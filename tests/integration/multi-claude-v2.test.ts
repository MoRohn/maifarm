 ...
 * Tests the Python Redis coordination client and multi_claude_v2.py
 */
...
describe('Multi-Claude V2 Python Integration', () => {
  let redis: Redis;
  const testPythonScript = path.join(process.cwd(), 'multi_claude_v2.py');
  const testRedisClient = path.join(process.cwd(), 'redis_coordination_client.py');
