/**
 * Quick Task Full Pipeline Test
 * Tests: Creation → Execution → Harvest → Barn Storage
 * Run this in the browser console to watch the entire process
 */
...
async function testQuickTaskFullPipeline() {
  ...
  try {
    ...
    const createResponse = await fetch('/api/quick-task/create', {
      method: 'POST',
    ...
    results.taskId = taskData.data?.id || taskData.taskId;
    ...
    while (!taskComplete && attempts < maxAttempts) {
      ...
      const statusResponse = await fetch(`/api/quick-task/status/${results.taskId}`);
    ...
    const harvestsResponse = await fetch(`/api/harvests/farms/quick-task-${results.taskId}`);
    if (harvestsResponse.ok) {
      ...
      results.errors.push(`Failed to fetch harvests: ${harvestsResponse.statusText}`);
    }

    // Step 4: Check Barn Storage
    console.log('\n🏚️ Step 4: Checking Barn Storage...');
    const barnResponse = await fetch('/api/barn/items');
    if (barnResponse.ok) {
      ...
      if (ourBarnItem) {
        ...
        if (results.harvestId) {
          ...
          const storeResponse = await fetch(`/api/barn/store/${results.harvestId}`, {
            method: 'POST'
          });
      ...
      results.errors.push(`Failed to fetch barn items: ${barnResponse.statusText}`);
    }

    // Step 5: Verify Terminal Sessions
    console.log('\n💻 Step 5: Checking Terminal Sessions...');
    const terminalResponse = await fetch(`/api/harvest/terminal/sessions?farmId=quick-task-${results.taskId}`);
    ...
    const coordResponse = await fetch('/api/harvest/coordination/agents');
    if (coordResponse.ok) {
  ...
  console.log('\n🔗 Quick Links:');
  if (results.taskId) {
    console.log(`  - Harvest Page: /harvest/quick-task-${results.taskId}`);
  }
  if (results.harvestId) {
    console.log(`  - Harvest API: /api/harvests/${results.harvestId}`);
  }
  if (results.barnItemId) {
    console.log(`  - Barn Item: /api/barn/items/${results.barnItemId}`);
  }
...
// Auto-run the test
console.log('Starting Quick Task Full Pipeline Test...');
console.log('This will create a real task and monitor it through completion.');
console.log('Please ensure the server is running on port 4567.');
console.log('');

// Add navigation helper
console.log('After the test completes, you can navigate to:');
console.log('  - Barn Page to see stored results');
console.log('  - Harvest Page to see live progress');
console.log('');

// Run the test
testQuickTaskFullPipeline().then(results => {
  // Store results globally for inspection
  window.quickTaskTestResults = results;
  console.log('\nTest results saved to: window.quickTaskTestResults');
  
  // Offer to navigate to relevant pages
  if (results.taskId) {
    console.log('\nTo view the harvest page, run:');
    console.log(`  window.location.href = '/harvest/quick-task-${results.taskId}'`);
  }
  console.log('\nTo view the barn page, run:');
  console.log("  window.location.href = '/barn'");
});