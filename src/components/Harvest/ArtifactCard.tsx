...
interface ArtifactCardProps {
  ...
  onView?: (artifact: HarvestArtifact) => void;
  onDownload?: (artifact: HarvestArtifact) => void;
  compact?: boolean;
}
...
export const ArtifactCard: React.FC<ArtifactCardProps> = ({ 
  ...
  const getArtifactIcon = () => {
    switch (artifact.type) {
      ...
      case 'file':
        if (artifact.mimeType?.includes('json')) return <FileJson className="w-5 h-5" />;
        if (artifact.mimeType?.includes('image')) return <FileImage className="w-5 h-5" />;
  ...
  return (
    ...
    >
      ...
      <div className={`h-2 bg-gradient-to-r ${
        artifact.type === 'code' ? 'from-blue-500 to-cyan-500' :
        artifact.type === 'documentation' ? 'from-green-500 to-emerald-500' :
        artifact.type === 'data' ? 'from-purple-500 to-pink-500' :
        artifact.type === 'model' ? 'from-orange-500 to-red-500' :
      ...
      <div className="p-6">
        ...
        <div className="space-y-2 mb-4">
          <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-500">
            ...
            <span>Created by {artifact.createdBy.agentName}</span>
          </div>
          <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-500">
            <FileCode className="w-3 h-3" />
            <span className="truncate" title={artifact.location}>
              {artifact.location}
            </span>
          </div>
        </div>
