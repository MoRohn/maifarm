...
export class FarmLaunchService {
  ...
  static async createQuickTask(description: string, attachedFiles?: File[]): Promise<any> {
    ...
    try {
      ...
      // Fallback: Create a local quick task ID
      if (error.response?.status >= 500 || !error.response) {
        const fallbackId = `qt-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
        console.warn('[FarmLaunchFix] Using fallback ID:', fallbackId);
        
        // Store for later sync
        const pendingTasks = JSON.parse(localStorage.getItem('pendingQuickTasks') || '[]');
        pendingTasks.push({
          farmId: fallbackId,
          description,
        ...
        localStorage.setItem('pendingQuickTasks', JSON.stringify(pendingTasks));
        
        return { farmId: fallbackId, harvestId: null };
      }
