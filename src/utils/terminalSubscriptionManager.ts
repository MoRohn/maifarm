...
class TerminalSubscriptionManager {
  ...
  private isSessionMatch(incomingSessionId: string, subscribedSessionId: string): boolean {
    // FIXED: Strict matching only - no fuzzy matching that causes cross-session leakage
    // Direct match is the only acceptable match
    if (incomingSessionId === subscribedSessionId) {
      return true;
    }

    // CRITICAL: Remove fuzzy matching to prevent output leakage between sessions
    // Each subscription should only receive outputs from its exact session
    
    // Special case: Handle exact session name variations for the SAME farm
    // But only if they clearly refer to the same entity
    const extractCoreId = (id: string) => {
      if (!id) return null;
      // Handle all prefixes and extract the core ID
      const withoutPrefix = id.replace(/^(farm-|quick_|goWild-|session-)/, '');
      // Take first 8 chars of the UUID for comparison
      return withoutPrefix.substring(0, 8).toLowerCase();
    };

    const incomingCore = extractCoreId(incomingSessionId);
    const subscribedCore = extractCoreId(subscribedSessionId);
    
    // Only match if BOTH have the same prefix type AND the same core ID
    // This prevents quick_ sessions from matching farm- sessions
    const incomingPrefix = incomingSessionId.match(/^(farm-|quick_|goWild-)/)?.[0] || '';
    const subscribedPrefix = subscribedSessionId.match(/^(farm-|quick_|goWild-)/)?.[0] || '';
    
    const matches = !!(
      incomingCore && 
      subscribedCore && 
      incomingCore === subscribedCore && 
      incomingPrefix === subscribedPrefix // Same prefix type required
    );
    
    if (matches) {
      console.log('[TerminalSubscriptionManager] Flexible match:', incomingSessionId, '≈', subscribedSessionId, '(core:', incomingCore, ')');
    }
