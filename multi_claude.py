...
class AgentLauncher:
    ...
    def launch_agent(self, agent_id: int, project_path: Path, debug: bool = False, provider: str = 'claude') -> Agent:
        """Launch a single AI Code agent (Claude or Qwen)"""
        ...
        if claude_check.returncode != 0:
            # Claude not installed - show message in pane
            print(f"[!] Agent {agent_id}: Claude Code not found - showing instructions")
            self.tmux.send_to_pane(agent_id, "echo '=== Claude Code Not Installed ==='")
            self.tmux.send_to_pane(agent_id, "echo 'To use Farm Creation with real Claude agents:'")
            self.tmux.send_to_pane(agent_id, "echo '1. Install Claude Code from: https://claude.ai/code'")
            self.tmux.send_to_pane(agent_id, "echo '2. Ensure claude command is in your PATH'")
            self.tmux.send_to_pane(agent_id, "echo ''")
            self.tmux.send_to_pane(agent_id, "echo 'For now, this pane is running in demo mode.'")
            self.tmux.send_to_pane(agent_id, "echo 'You can type commands here manually.'")
            self.tmux.send_to_pane(agent_id, "echo ''")
        ...
        if provider == 'qwen':
            # For Qwen, we have multiple options:
            # 1. Use Ollama for local model
            # 2. Use Qwen API with proxy
            # 3. Use direct Qwen integration
            ...
            if ollama_check.returncode == 0 and 'qwen' in ollama_check.stdout.lower():
                ...
                time.sleep(0.5)
                # Launch with Qwen-specific wrapper
                self.tmux.send_to_pane(agent_id, "qwen-code --local --dangerously-skip-permissions")
            else:
                # Fall back to API proxy method
                print(f"[*] Agent {agent_id}: Using Qwen API via proxy")
    ...
    def run(self):
        ...
        try:
            ...
            for i in range(self.args.agents):
                ...
                self.launcher.launch_agent(i, self.project_path, debug=self.debug, provider=self.provider)
