...
async function testGoWildWorkflow() {
  ...
  try {
    ...
    console.log('\n6️⃣ Testing Socket.IO connection...');
    ...
    await new Promise((resolve, reject) => {
      socket.on('connect', () => {
        ...
        socket.emit('harvest:join', { harvestId, farmId });
    ...
    socket.on('farm:status', (data) => {
      console.log(`📨 Farm status:`, JSON.stringify(data).substring(0, 100));
    });
    socket.on('harvest:status', (data) => {
      console.log(`📨 Harvest status:`, JSON.stringify(data).substring(0, 100));
    });
    socket.on('agent:status', (data) => {
      console.log(`📨 Agent status:`, JSON.stringify(data).substring(0, 100));
    });
