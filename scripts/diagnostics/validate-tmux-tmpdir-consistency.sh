...
if grep -q "const defaultTmuxTmpDir = '/tmp'" ${MAIFARM_ROOT}/apps/api/src/config/paths.ts; then
...
if grep -q "process.env.TMUX_TMPDIR = config.TMUX_TMP_DIR" ${MAIFARM_ROOT}/apps/api/src/config/paths.ts; then
...
if grep -q "process.env.TMUX_TMPDIR || '/tmp'" ${MAIFARM_ROOT}/apps/api/src/config/paths.ts; then
...
TMUX_COMMANDS_WITHOUT_TMPDIR=$(grep -r "tmux " ${MAIFARM_ROOT}/apps/api/src/services/ | grep -v "TMUX_TMPDIR" | grep -v ".test.ts" | grep -v "Binary file" | wc -l)
...
if grep -q "SESSION_TIMEOUT = 600000" ${MAIFARM_ROOT}/apps/api/src/services/unified/terminalService.ts; then
...
if grep -q 'env\["TMUX_TMPDIR"\] = "/tmp"' ${MAIFARM_ROOT}/scripts/python/orchestrator.py; then
...
PRIVATE_TMP_USAGE=$(grep -r "/private/tmp" ${MAIFARM_ROOT}/apps/api/src/ 2>/dev/null | grep -v ".test.ts" | wc -l)
...
if grep -q "const tmuxTmpDir = pathConfig.getPath('TMUX_TMP_DIR')" ${MAIFARM_ROOT}/apps/api/src/services/UnifiedFarmLaunchOrchestrator.ts; then
