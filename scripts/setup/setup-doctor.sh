...
check_permissions() {
    ...
    scripts=("scripts/dev/startup.sh" "scripts/setup/setup-all.sh" "scripts/setup/setup-postgres.sh")
...
auto_fix() {
    ...
    if [ -f "scripts/dev/startup.sh" ] && [ ! -x "scripts/dev/startup.sh" ]; then
        chmod +x scripts/dev/startup.sh
        log_success "Fixed permissions for startup.sh"
    fi
    
    if [ -f "scripts/setup/setup-all.sh" ] && [ ! -x "scripts/setup/setup-all.sh" ]; then
        chmod +x scripts/setup/setup-all.sh
        log_success "Fixed permissions for setup-all.sh"
    fi
    
    if [ -f "scripts/setup/setup-postgres.sh" ] && [ ! -x "scripts/setup/setup-postgres.sh" ]; then
        chmod +x scripts/setup/setup-postgres.sh
