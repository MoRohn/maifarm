#!/usr/bin/env python3
"""
XenoSync CLI Orchestrator

A simplified tool to run multiple Claude Code (XenoSync) sessions in parallel
for collaborative problem-solving. Inspired by claude_code_agent_farm and Builder.

Usage:
    poetry run maifarm-xenosync --num-agents 5 --prompt "Fix all TypeScript errors"
    poetry run maifarm-xenosync --num-agents 3 --prompt-file feature.yaml
    poetry run maifarm-xenosync --num-agents 4 --prompt "Refactor API" --steps "Analyze" "Implement" "Test"
"""
...
class CoordinationManager:
    ...
    def __init__(self, project_path: Path):
        ...
        maibarn_root = os.environ.get('MAIBARN_ROOT', Path.cwd() / 'var' / 'maibarn')
...
class MultiClaudeOrchestrator:
    ...
    def _parse_prompt(self) -> BuildPrompt:
        """Parse prompt from args or workspace YAML"""
        # First check if farm.yaml exists in workspace (from YAML generation)
        workspace_yaml = self.workspace_path / 'farm.yaml'
        if workspace_yaml.exists():
            print(f"[+] Found farm.yaml in workspace, using for coordination")
            return PromptParser.parse_file(workspace_yaml)

        # Otherwise use explicit prompt file
...
def main():
    parser = argparse.ArgumentParser(
        description="Run multiple XenoSync agents in parallel",
        epilog="""
Examples:
  # Single prompt to all agents
  poetry run maifarm-xenosync --agents 5 --prompt "Fix all TypeScript errors in the codebase"
  
  # With coordination instructions
  poetry run maifarm-xenosync --agents 3 --prompt "Refactor the API layer" --coordinate
  
  # Step-based workflow
  poetry run maifarm-xenosync --agents 4 --prompt "Build feature X" --steps "Design" "Implement" "Test"
  
  # From prompt file
  poetry run maifarm-xenosync --agents 4 --prompt-file feature.yaml
        """,
