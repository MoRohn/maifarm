...
![MaiFarm Logo](apps/dashboard/public/maifarm-logo-dark-bkgd.svg)
...
pyproject.toml   # Poetry definition for Python orchestration tools
ops/
  config/        # Configuration templates (YAML, Docker)
  deployment/    # Deployment scripts and manifests
  docker/        # Docker assets (Dockerfiles, compose files)
  infrastructure/# Infrastructure as code assets
  nginx/         # Reverse proxy configuration
  process/       # PM2 and process management configs
scripts/         # Automation and operational scripts
development/     # Examples, manual tests, and scaffolding templates
docs/            # Architecture, provider guides, API references
```
...
MaiFarm ships several orchestration helpers in `scripts/python/`. Python dependencies are managed with [Poetry](https://python-poetry.org/):
...
# or use the official installer with a Homebrew Python
curl -sSL https://install.python-poetry.org | /opt/homebrew/bin/python3 -

# Install dependencies declared in pyproject.toml
poetry install

# Execute a helper inside the Poetry environment
poetry run maifarm-orchestrator --help
```

Poetry creates an isolated virtual environment in `.venv/` (ignored by git). If you already have system-level tooling such as PostgreSQL installed, ensure the corresponding binaries are on your `PATH` when invoking `poetry run` commands.
...
- [Production Deployment](ops/deployment/) - Docker, monitoring, scaling guides  
- [Development Resources](development/) - Templates, manual test fixtures, examples, and tools
...
curl -X POST http://localhost:4567/api/go-wild \
  ...
  -d '{
    ...
    "creativity": 0.8,
    "boundaries": ["apps/dashboard/src/", "apps/api/src/"],
    "timeout": 1800
  }'
```
...
│
├── ops/
│   ├── config/         # YAML templates and starter configs
│   ├── deployment/     # Deployment scripts and manifests
│   ├── docker/         # Dockerfiles and compose bundles
│   ├── infrastructure/ # Infrastructure as code assets
│   ├── nginx/          # Reverse-proxy configuration
│   └── process/        # PM2 and process management configs
│
├── development/        # Examples, templates, manual test fixtures
├── docs/               # Architecture notes, provider guides, ADRs
├── scripts/            # Automation scripts (setup, diagnostics, tooling)
├── tests/              # Unit, integration, e2e, and performance suites
├── tools/              # Shared build, testing, and database tooling
├── packages/           # Additional utilities (reserved/experimental)
├── var/                # Git-ignored runtime artefacts (logs, backups)
...
sudo cp ops/nginx/maifarm.conf /etc/nginx/sites-available/
