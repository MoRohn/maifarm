...
echo ""
echo "2. Testing multi_claude.py directly..."
timeout 30 python3 multi_claude.py -n 2 -p "Test: Print 'Hello from agent'" --debug --no-kill-on-exit &
PID=$!
...
if ps -p $PID > /dev/null; then
    echo "   ✓ multi_claude.py is running (PID: $PID)"
    ...
    echo "   ✗ multi_claude.py exited early"
fi
