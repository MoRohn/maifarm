...
const terminalOutputWatcher = {
  watch: (sessionName: string) => terminalService.watchTerminal(sessionName),
  stopWatching: (sessionName: string) => terminalService.stopWatchingTerminal(sessionName)
};
import { terminalService } from '../services/unified/terminalService';
...
const detectWindowTarget = async (sessionName: string): Promise<string> => {
  try {
    ...
    console.warn(`[TerminalHandlers] Could not detect window target for ${sessionName}, defaulting to '0':`, error.message);
    return '0';
  }
};
...
export const createTerminalWebSocketHandlers = (ioInstance: SocketServer): TerminalWebSocketHandlers => {
  ...
  const handleTerminalHealthPong = (socket: Socket, data: { sessionName: string; agentIndex: number }) => {
    ...
    // Update coordinator with health status
    terminalViewCoordinator.handleHealthPong(sessionName, agentIndex);
  ...
  const handleTerminalViewSwitch = async (socket: Socket, data: { farmId: string; viewType: 'grid' | 'stacked' | 'single'; selectedAgent?: number }) => {
    ...
    // Handle view switch via coordinator
    await terminalViewCoordinator.switchView(farmId, viewType, selectedAgent);
    ...
    setTimeout(async () => {
      ...
      if (sessionName) {
        ...
        try {
          ...
          logThrottling.error(`capture-existing-error-${sessionId}`, `Error capturing existing output: ${error.message}`);
        }
      }
  ...
  const stopOutputMonitoring = async (sessionId: string) => {
    console.log(`[TerminalHandlers] Stopping output monitoring for session ${sessionId}`);
    
    // Find the actual session name if it differs
    const { sessionName } = await findActualSessionName(sessionId);
    const sessionToStop = sessionName || sessionId;
    
    // Stop the terminal output watcher
    if (terminalOutputWatcher.isWatching(sessionToStop)) {
      terminalOutputWatcher.stopWatching(sessionToStop);
    }
  ...
  const getSessionAgentCount = async (sessionId: string): Promise<number> => {
    ...
    return new Promise((resolve) => {
      ...
      countPanes.on('error', (error) => {
        console.error(`[TerminalHandlers] Error counting panes for ${actualSession}:`, error.message);
        resolve(0);
      });
    });
  };
  ...
  const captureAgentOutput = async (sessionId: string, agentId: number, farmId?: string): Promise<string[]> => {
    ...
    return new Promise((resolve) => {
      ...
      capture.on('error', (error) => {
        console.error(`[TerminalHandlers] Error capturing output from ${paneTarget}:`, error.message);
        resolve([]);
      });
    });
  };
