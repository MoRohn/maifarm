...
from .terminal_manager import IndividualTerminalManager
...
class XenosyncOrchestrator:
    ...
    def __init__(self, config: Config, session_manager: SessionManager, 
        ...
        # Get terminal mode from config
        self.terminal_mode = config.get('terminal_mode', 'tmux')
        
        # Initialize terminal managers based on mode
        self.tmux_manager = None
        self.individual_terminal_manager = None
        
        if self.terminal_mode == 'separate':
            # Individual terminal windows for each agent
            self.individual_terminal_manager = IndividualTerminalManager(config._config)
            logger.info("Using individual terminal windows mode")
        elif self.terminal_mode == 'tmux-windows':
            # Tmux with separate windows
            if config.get('use_tmux', True):
                session_name = config.get('tmux_session_name', 'xenosync-hive')
                self.tmux_manager = TmuxManager(session_name, separate_windows=True)
                logger.info(f"Using tmux separate windows mode with session: {session_name}")
        else:
            # Default tmux panes mode
            if config.get('use_tmux', True):
                session_name = config.get('tmux_session_name', 'xenosync-hive')
                self.tmux_manager = TmuxManager(session_name, separate_windows=False)
                logger.info(f"Using tmux panes mode with session: {session_name}")
        ...
        self.preserve_tmux_session = False  # Flag to preserve tmux sessions for MaiFarm
    ...
    def _cleanup_tmux_sessions(self):
        """Clean up tmux sessions or individual terminals (can be called from signal handler)"""
        try:
            # Check if we should preserve tmux sessions for MaiFarm
            if self.preserve_tmux_session:
                logger.info("Preserving tmux session for MaiFarm - skipping cleanup")
                return
            
            if self.individual_terminal_manager:
                logger.info("Cleaning up individual terminal windows")
                self.individual_terminal_manager.close_all_terminals()
            
            if self.tmux_manager:
                logger.info("Cleaning up tmux session from signal handler")
                self.tmux_manager.kill_session()
            
            # Safety net - kill any xenosync sessions (skip if preserving)
            if not self.preserve_tmux_session:
                from .tmux_manager import TmuxManager
    ...
    async def run(self, session: Session, prompt: SyncPrompt):
        ...
        try:
            ...
            # Set up terminal visualization based on mode
            if self.terminal_mode == 'separate':
                # Individual terminals - set up manager
                self.agent_manager.set_individual_terminal_manager(self.individual_terminal_manager)
                logger.info(f"Will open {self.num_agents} individual terminal windows")
            elif self.tmux_manager and self.tmux_manager.is_tmux_available():
                # Tmux modes (panes or windows)
            ...
            # Pass the initial prompt to agents so Claude starts with context
            initial_prompt = prompt.initial_prompt if prompt else None
            await self.agent_manager.initialize_agents(session.id, initial_prompt)
