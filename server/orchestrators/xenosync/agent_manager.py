...
class AgentManager:
    ...
    def __init__(self, config: Config, num_agents: int = 2):
        ...
        self.individual_terminal_manager = None  # Will be set for individual terminals
    ...
    def set_individual_terminal_manager(self, terminal_manager):
        """Set the individual terminal manager for separate windows"""
        self.individual_terminal_manager = terminal_manager
    ...
    async def initialize_agents(self, session_id: str, initial_prompt: Optional[str] = None) -> List[Agent]:
        """Initialize all agent instances with optional initial prompt"""
        logger.info(f"Initializing {self.num_agents} agents for session {session_id}")
        self.initial_prompt = initial_prompt  # Store for use in agent creation
        
        for i in range(self.num_agents):
            agent = await self._create_agent(i, session_id, initial_prompt)
    ...
    async def _create_agent(self, agent_id: int, session_id: str, initial_prompt: Optional[str] = None) -> Agent:
        """Create and start a single agent with optional initial prompt"""
        # Generate unique ID
        ...
        # Configure terminal mode for the interface
        if self.individual_terminal_manager:
            # Individual terminal mode
            interface.set_individual_terminal_mode()
        elif self.tmux_manager:
            # Tmux pane mode
        ...
        try:
            if self.individual_terminal_manager:
                # Open individual terminal for this agent
                claude_command = ' '.join([self.config.get('claude_command', 'claude')] + 
                                         self.config.get('claude_args', ['--dangerously-skip-permissions']))
                
                success = self.individual_terminal_manager.open_agent_terminal(
                    agent_id=agent_id,
                    working_directory=agent.worktree_path or os.getcwd(),
                    claude_command=claude_command,
                    session_id=session_id
                )
                
                if success:
                    await interface.start(f"{session_id}_agent_{agent_id}", agent_uid=uid, prompt=initial_prompt)
                    agent.status = AgentStatus.WORKING
                    agent.update_activity()
                    logger.info(f"Agent {agent_id} ({uid}) started in individual terminal")
                else:
                    raise AgentError(f"Failed to open terminal for agent {agent_id}")
            else:
                # Standard tmux or direct mode
                await interface.start(f"{session_id}_agent_{agent_id}", agent_uid=uid, prompt=initial_prompt)
