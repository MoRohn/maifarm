...
@click.option('--days', '-d', type=int, default=7, 
              help='Clean sessions older than this many days')
@click.option('--all', is_flag=True, help='Remove all sessions')
@click.option('--dry-run', is_flag=True, help='Show what would be deleted without deleting')
@click.pass_context
def cleanup(ctx, days, all, dry_run):
    """Clean up old Xenosync sessions to free disk space"""
    import shutil
    from datetime import datetime, timedelta
    
    config = ctx.obj['config']
    sessions_dir = Path(config.get('sessions_dir', 'xsync-sessions'))
    ...
    click.echo(f"{'[DRY RUN] ' if dry_run else ''}Scanning sessions...")
    ...
    for session_dir in sessions_dir.iterdir():
        ...
        should_remove = all or mod_time < cutoff_date
        ...
        if should_remove:
            ...
            if dry_run:
                click.echo(f"Would remove: {session_dir.name} ({size/1024:.1f} KB, modified {mod_time:%Y-%m-%d})")
            else:
                click.echo(f"Removing: {session_dir.name} ({size/1024:.1f} KB)")
                shutil.rmtree(session_dir)
            
            sessions_removed += 1
    
    if sessions_removed == 0:
        click.echo(f"No sessions {'would be' if dry_run else 'were'} removed")
    else:
        click.echo(f"\n{'Would remove' if dry_run else 'Removed'} {sessions_removed} session(s)")
        click.echo(f"{'Would free' if dry_run else 'Freed'} {total_size/1024/1024:.2f} MB of disk space")
...
@click.option('--terminal-mode', type=click.Choice(['tmux', 'tmux-windows', 'separate']), 
              help='Terminal mode: tmux (panes), tmux-windows, or separate terminals')
@click.pass_context
def start(ctx, prompt_file, agents, resume, dry_run, no_terminal, terminal_mode):
    ...
    if terminal_mode:
        config.set('terminal_mode', terminal_mode)
