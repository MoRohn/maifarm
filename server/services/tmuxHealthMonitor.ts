...
export class TmuxHealthMonitor extends EventEmitter {
  ...
  private readonly STARTUP_DELAY = 5000; // 5 seconds startup delay
  private readonly STARTUP_GRACE_PERIOD = 30000; // 30 seconds grace period for new farms
  private readonly MAX_CONSECUTIVE_FAILURES = 3; // Require 3 failures before marking as crashed
  ...
  private async checkSessionHealth(
    ...
    const sessionData = existingSessions.get(farm.sessionName);
    ...
    // Get pane count
    const paneCount = await this.getSessionPaneCount(farm.sessionName);
  ...
  private async handleDeadSession(farm: FarmSession, health: SessionHealth): Promise<void> {
    logger.error(`[TmuxHealthMonitor] CRITICAL: Session ${farm.sessionName} for farm ${farm.farmId} is DEAD`);
