  ...
  public watchDirectory(dirPath: string, eventName: string) {
    ...
    try {
      if (!existsSync(dirPath)) {
        console.log(`[FileWatcher] Directory does not exist: ${dirPath}`);
        return;
      }
      ...
      console.log(`[FileWatcher] Now watching directory: ${dirPath}`);
    } catch (error) {
  ...
  private setupWatcher(watchedFile: WatchedFile) {
    try {
      ...
      if (!existsSync(watchedFile.path)) {
        console.log(`[FileWatcher] File does not exist yet: ${watchedFile.path}`);
        // Will retry in periodic check
        return;
      }
  ...
  private async readFileContent(watchedFile: WatchedFile) {
    try {
      ...
      if (error.code === 'ENOENT') {
        // File doesn't exist
        console.log(`[FileWatcher] File not found: ${watchedFile.path}`);
