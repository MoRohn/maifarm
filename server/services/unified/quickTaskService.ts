...
class UnifiedQuickTaskService extends EventEmitter {
  ...
  private readonly AGENT_COUNT = 2; // Quick tasks always use 2 agents
  ...
  private enforceTimeout(task: QuickTaskInternal): void {
    // Set hard timeout at exactly QUICK_TASK_TIMEOUT
    task.timeoutHandle = setTimeout(async () => {
      logger.warn(`Quick task ${task.id} reached timeout (${QUICK_TASK_TIMEOUT}ms)`);
      ...
      await shutdownCoordinator.initiateShutdown(task.farmId, 'timeout');
