...
class UnifiedAIProviderService extends EventEmitter {
  ...
  private async loadApiKeys(): Promise<void> {
    try {
      ...
      for (const row of result.rows) {
        try {
          ...
          } else if (row.service.toLowerCase() === 'qwen') {
            this.apiKeys.set(AIProvider.QWEN, decryptedKey);
          }
        } catch (decryptError) {
    ...
    if (!this.apiKeys.has(AIProvider.QWEN)) {
      const qwenKey = process.env.QWEN_API_KEY;
      if (qwenKey) {
        this.apiKeys.set(AIProvider.QWEN, qwenKey);
