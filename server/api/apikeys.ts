    ...
    try {
      ...
      await db.query(`
        ...
        DO UPDATE SET 
          ...
          is_active = true
      `, ['claude', encrypted, hashed, name || 'Claude API Key', ['read', 'write']]);
...
router.post('/apikeys', async (req: Request, res: Response) => {
  try {
    ...
    if (service.toLowerCase() === 'claude') {
      process.env.ANTHROPIC_API_KEY = apiKey;
      process.env.CLAUDE_API_KEY = apiKey;
    } else if (service.toLowerCase() === 'openai') {
      process.env.OPENAI_API_KEY = apiKey;
    } else if (service.toLowerCase() === 'qwen') {
    ...
    await db.query(`
      ...
      DO UPDATE SET
        ...
        last_used = CURRENT_TIMESTAMP,
        is_active = true
    `, [service.toLowerCase(), encrypted, hashed, name || `${service} API Key`, ['read', 'write']]);
