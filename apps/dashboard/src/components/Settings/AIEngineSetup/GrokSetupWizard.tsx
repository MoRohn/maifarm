...
interface GrokSetupWizardProps {
  isOpen: boolean;
  onClose: () => void;
  onComplete: ((apiKey: string, model: string) => void) | ((result: EngineConfigResult) => void);
}

export const GrokSetupWizard: React.FC<GrokSetupWizardProps> = ({
  isOpen,
  onClose,
  onComplete,
}) => {
  // Default model for Grok
  const defaultGrokModel = PROVIDER_MODELS.grok?.[0]?.id ?? 'grok-2';

  const [apiKey, setApiKey] = useState('');
  const [model, setModel] = useState(defaultGrokModel);
  const [isSaving, setIsSaving] = useState(false);
  const [validation, setValidation] = useState<ValidationResult | undefined>();
  const [isValidating, setIsValidating] = useState(false);
  const { success: showSuccess, error: showError } = useToast();

  const colors = PROVIDER_COLORS.grok;
  const models = PROVIDER_MODELS.grok ?? [];
  const docs = PROVIDER_DOCS.grok ?? { apiKeys: 'https://console.x.ai', models: 'https://x.ai/api', quickstart: 'https://docs.x.ai' };
  const formatHint = API_KEY_FORMATS.grok ?? { prefix: ['xai-', 'grok-'], description: 'Grok API keys start with "xai-" or "grok-"' };

  useEffect(() => {
    if (!isOpen) {
      setApiKey('');
      setModel(defaultGrokModel);
      setIsSaving(false);
      setValidation(undefined);
      setIsValidating(false);
    } else {
      // Load saved model preference
      const savedModel = localStorage.getItem(STORAGE_KEYS.GROK_MODEL);
      if (savedModel) {
        setModel(savedModel);
      }
    }
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(async () => {
      ...
      try {
        const result = await engineConfigService.validateApiKey('grok', apiKey);
        if (!cancelled) {
  ...
  const handleComplete = async () => {
    ...
    if (!trimmed) {
      showError(ERROR_MESSAGES.API_KEY_REQUIRED('Grok'));
      return;
    }

    // Check format before submitting
    if (!formatHint.prefix.some(prefix => trimmed.startsWith(prefix))) {
      showError(ERROR_MESSAGES.API_KEY_FORMAT('Grok', formatHint.prefix.join(' or ')));
      return;
    }
    ...
    try {
      ...
      const result = await engineConfigService.configure('grok', {
        apiKey: trimmed,
        model,
      });

      // Save model preference
      localStorage.setItem(STORAGE_KEYS.GROK_MODEL, model);

      showSuccess(getSuccessMessage('Grok'));
      ...
      {isOpen && (
        ...
        >
          <motion.div
            role="dialog"
            aria-modal="true"
            aria-labelledby="grok-setup-title"
          ...
          >
            ...
            <div className="flex items-start justify-between gap-3 border-b border-gray-200 p-6 dark:border-gray-700">
              <div>
                <h2 id="grok-setup-title" className="text-xl font-semibold text-gray-900 dark:text-white">
                  Connect Grok (xAI)
                </h2>
                <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
                  Connect your xAI account to power your AI agents with Grok-2.
                </p>
              </div>
              <button
                type="button"
              ...
              <div className="space-y-4">
                ...
                <div className="rounded-lg border border-orange-200 bg-orange-50 p-3 dark:border-orange-800 dark:bg-orange-900/20">
                  <div className="flex items-start gap-2">
                    <InformationCircleIcon className="h-5 w-5 flex-shrink-0 text-orange-600 dark:text-orange-400" />
