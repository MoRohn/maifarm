...
export const OpenAISetupWizard: React.FC<OpenAISetupWizardProps> = ({
  ...
  const colors = PROVIDER_COLORS.openai;
  const models = openaiModels;
  const docs = PROVIDER_DOCS.openai ?? { apiKeys: '', models: '', quickstart: '' };
  const formatHint = API_KEY_FORMATS.openai ?? { prefix: [], description: '' };

  useEffect(() => {
    if (!isOpen) {
      setApiKey('');
      setModel(openaiModels[0]?.id ?? '');
      setIsSaving(false);
      setValidation(undefined);
      setIsValidating(false);
    } else {
      // Load saved model preference
      const savedModel = localStorage.getItem(STORAGE_KEYS.OPENAI_MODEL);
      if (savedModel) {
        setModel(savedModel);
      }
    }
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(async () => {
      ...
      try {
        const result = await engineConfigService.validateApiKey('openai', apiKey);
        if (!cancelled) {
  ...
  const handleComplete = async () => {
    ...
    if (!trimmed) {
      showError(ERROR_MESSAGES.API_KEY_REQUIRED('OpenAI'));
      return;
    }

    // Check format before submitting
    if (!formatHint.prefix.some(prefix => trimmed.startsWith(prefix))) {
      showError(ERROR_MESSAGES.API_KEY_FORMAT('OpenAI', formatHint.prefix.join(' or ')));
      return;
    }
    ...
    try {
      ...
      const result = await engineConfigService.configure('openai', {
        apiKey: trimmed,
        model,
      });
      ...
      showSuccess(getSuccessMessage('OpenAI'));
      ...
      {isOpen && (
        ...
        >
          <motion.div
            role="dialog"
            aria-modal="true"
            aria-labelledby="openai-setup-title"
          ...
          >
            ...
            <div className="flex items-start justify-between gap-3 border-b border-gray-200 p-6 dark:border-gray-700">
              <div>
                <h2 id="openai-setup-title" className="text-xl font-semibold text-gray-900 dark:text-white">
                  Connect OpenAI
                </h2>
                <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
                  Connect your OpenAI account to power your AI agents with GPT-4.
                </p>
              </div>
              <button
                type="button"
