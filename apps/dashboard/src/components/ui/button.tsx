...
// FIX: Updated to meet Apple HIG 44px minimum touch target
const buttonSizes = {
  default: 'h-11 px-5 py-2.5',  // 44px (Apple HIG minimum)
  sm: 'h-9 px-3 text-xs',        // 36px (acceptable for compact UI)
  lg: 'h-12 px-8 text-base',     // 48px
  icon: 'h-11 w-11',             // 44px (Apple HIG minimum)
};
...
export const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className = '', variant = 'default', size = 'default', asChild = false, ...props }, ref) => {
    ...
    return (
      <Comp
        className={`inline-flex items-center justify-center rounded-apple text-sm font-medium focus:outline-none focus:ring-2 focus:ring-primary/50 focus:ring-offset-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50 focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 touch-manipulation ${buttonVariants[variant]} ${buttonSizes[size]} ${className}`}
        ref={ref as any}
        {...(asChild ? {} : props)}
      >
        {props.children}
