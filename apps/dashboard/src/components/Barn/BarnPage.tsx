...
import { ArchivedFarmsView } from './ArchivedFarmsView';
import { ArchivedFarmDetails } from './ArchivedFarmDetails';
...
export const BarnPage: React.FC = () => {
  ...
  const loadBarnData = async () => {
    ...
    try {
      ...
      const allHarvests = Array.isArray(itemsRes.data) ? itemsRes.data : [];
      setHarvests(allHarvests);

      // Only process new harvests if there are any
      ...
      // Handle folders response - ensure it's an array and harvestIds is initialized
      const folderData = Array.isArray(foldersRes.data) ? foldersRes.data : [];
      const normalizedFolders = folderData.map(folder => ({
        ...folder,
        harvestIds: folder.harvestIds || []
      }));
  ...
  const filteredHarvests = harvests.filter(harvest => {
    // Ensure harvest exists and has required properties
    if (!harvest || !harvest.id) return false;

    // Only exclude empty harvests, not ones with valid content
    const hasValidContent = harvest.yield && Array.isArray(harvest.yield) && harvest.yield.length > 0;
    ...
    const harvestName = harvest.name || '';
    const harvestDesc = harvest.description || '';
    const matchesSearch = harvestName.toLowerCase().includes(searchQuery.toLowerCase()) ||
                         harvestDesc.toLowerCase().includes(searchQuery.toLowerCase());
  ...
  const handleCreateFolder = async (name: string) => {
    try {
      ...
      if (response.data.success) {
        const newFolder = {
          ...response.data.data,
          harvestIds: response.data.data.harvestIds || []
        };
    ...
    <div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="bg-white dark:bg-gray-900 rounded-apple-lg p-6 border border-gray-200 dark:border-gray-800">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-2xl font-bold text-gray-900 dark:text-white">
                  {stats?.totalItems || stats?.totalHarvests || 0}
                </span>
              </div>
            ...
            return (
              <button
                ...
                className={clsx(
                  ...
                  filterType === type && !showArchivedFarms
                    ? 'bg-primary-600 text-white'
                    : 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700'
                )}
              >
        ...
        {loading ? (
          ...
          <>
            {/* Show Archived Farms View */}
            {showArchivedFarms ? (
              <ArchivedFarmsView onSelectArchive={setSelectedArchiveId} />
            ) : (
                ...
                {folders
                  ...
                  .map((folder) => (
                    <FolderView
                      key={folder.id}
                      folder={folder}
                      harvestCount={folder.harvestIds?.length || 0}
                      onClick={() => {
                        setSelectedFolder(folder);
