...
apiClient.interceptors.response.use(
  ...
  async (error: AxiosError) => {
    ...
    try {
      ...
      // Handle 429 Rate Limit Exceeded with enhanced error details
      if ((retryError as AxiosError).response?.status === 429) {
        const errorData = (retryError as AxiosError).response?.data as any;

        // Create enhanced error with provider information
        const enhancedError = new Error(errorData?.error?.userMessage || errorData?.error?.message || 'Rate limit exceeded') as any;
        enhancedError.code = 'RATE_LIMIT_EXCEEDED';
        enhancedError.provider = errorData?.provider || 'claude';
        enhancedError.providerName = errorData?.providerName || 'Claude';
        enhancedError.providerUrl = errorData?.providerUrl || 'https://console.anthropic.com/settings/billing';
        enhancedError.actionRequired = errorData?.error?.actionRequired || 'add_credits';
        enhancedError.retryAfter = errorData?.error?.retryAfter;
        enhancedError.response = retryError.response;

        // Log for debugging
        console.error('[API] Rate limit exceeded:', {
          provider: enhancedError.provider,
          providerName: enhancedError.providerName,
          providerUrl: enhancedError.providerUrl,
          message: enhancedError.message
        });

        // Store error details for UI components to use
...
export const api = {
  ...
  quickActions: {
    createFarm: (data: any) => {
      ...
      return apiClient.post('/api/quick-actions/farm', data, config);
    },
    startGoWild: (data: any) => {
      const config = data instanceof FormData ? { headers: { 'Content-Type': 'multipart/form-data' } } : undefined;
      return apiClient.post('/api/quick-actions/go-wild', data, config);
    },
  },
  ...
  // Barn
  barn: {
    // Main barn item operations
    list: (filter?: any) => apiClient.get('/api/barn/items', { params: filter }),
    items: {
      list: (filter?: any) => apiClient.get('/api/barn/items', { params: filter }),
      get: (id: string) => apiClient.get(`/api/barn/items/${id}`),
      create: (data: any) => apiClient.post('/api/barn/items', data),
      update: (id: string, data: any) => apiClient.put(`/api/barn/items/${id}`, data),
      delete: (id: string) => apiClient.delete(`/api/barn/items/${id}`),
      use: (id: string, data?: any) => apiClient.post(`/api/barn/items/${id}/use`, data),
      archive: (id: string) => apiClient.post(`/api/barn/items/${id}/archive`),
    },
    // Stats and sync operations
