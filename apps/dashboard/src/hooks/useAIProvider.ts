    ...
    try {
      ...
      if (response.ok) {
        ...
        if (result.success && result.data.providers) {
          ...
          const selectedProvider = (result.data.currentProvider || result.data.defaultProvider || 'claude') as AIProvider;
          localStorage.setItem('ai_provider', selectedProvider);
          if (selectedProvider !== provider) {
            updateSettings({ aiProvider: selectedProvider } as any);
          }

          const currentStatus = status[selectedProvider] || { configured: true, enabled: true };
          setIsConfigured(currentStatus.configured && currentStatus.enabled);
        }
      }
    } catch (error) {
  ...
  const setProvider = useCallback((newProvider: AIProvider) => {
    ...
    fetch('/api/providers/default', {
      ...
      if (!response.ok) {
        ...
        console.error('Failed to set AI provider:', error?.error || response.statusText);
        updateSettings({ aiProvider: previous } as any);
        localStorage.setItem('ai_provider', previous);
