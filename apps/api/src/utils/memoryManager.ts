...
export class MemoryManager extends EventEmitter {
  ...
  private registerDefaultCleanupHandlers(): void {
    ...
    // Clear expired cache entries - DISABLED (module removed)
    // Note: terminalOutputCache service was removed, cleanup handled by terminalStreamService

    // Trigger garbage collection if available
    this.registerCleanupHandler('gc_cleanup', () => {
      if (global.gc) {
        global.gc();
        logger.debug(LogCategory.MONITORING, 'Triggered garbage collection');
      }
    }, 20);
