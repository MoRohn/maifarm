...
// GET /api/websocket/farm/:farmId - Get farm-specific room health
router.get('/farm/:farmId', async (req: Request, res: Response) => {
  try {
    ...
    const shortFarmRoom = `farm-${farmId.substring(0, 8)}`;

    // Get sockets in farm rooms
    const socketsInFarmRoom = wsServer.io.sockets.adapter.rooms.get(farmRoom);
    const socketsInShortRoom = wsServer.io.sockets.adapter.rooms.get(shortFarmRoom);

    const totalSockets = new Set([
      ...(socketsInFarmRoom || []),
      ...(socketsInShortRoom || [])
    ]);
    ...
    res.json({
      ...
      rooms: {
        [farmRoom]: {
          socketCount: socketsInFarmRoom?.size || 0,
          socketIds: Array.from(socketsInFarmRoom || [])
        },
        [shortFarmRoom]: {
          socketCount: socketsInShortRoom?.size || 0,
          socketIds: Array.from(socketsInShortRoom || [])
        }
      },
      totalUniqueConnections: totalSockets.size,
      socketIds: Array.from(totalSockets),
      timestamp: new Date(),
      status: totalSockets.size > 0 ? 'connected' : 'no_clients'
    });
  } catch (error) {
    console.error('Failed to get farm WebSocket health:', error);
    res.status(500).json({
      error: 'Failed to retrieve farm WebSocket health'
    });
  }
});
