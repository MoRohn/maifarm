...
router.get('/', async (req, res) => {
  try {
    ...
    const providerSummaries = providers.map((config) => {
      ...
      return {
        id: config.provider,
        name: config.provider === AIProvider.CLAUDE ? 'Claude' : config.provider,
        status: config.apiKey ? 'available' : 'not_configured',
        configured: config.apiKey !== '',
        models: config.provider === AIProvider.CLAUDE ? ['claude-3-sonnet-20240229'] : []
      };
    });
...
router.post('/default', authenticateToken, apiRateLimits.write, async (req, res) => {
  try {
    ...
    try {
      ...
      console.warn('[Providers] Failed to persist AI provider selection:', error);
    }
    ...
    const providerStatus = await Promise.all(
      providers.map(async (config) => {
        ...
        return {
          ...
          name: config.provider === AIProvider.CLAUDE ? 'Claude Code' : config.provider,
