...
export interface PathConfig {
  ...
  HARVEST_DIR: string; // Legacy alias for shared harvest storage
  HARVESTS_DIR: string; // Legacy alias for shared harvest storage
  ...
  BARN_CATALOG: string;
  BARN_DIR: string; // Legacy alias for barn storage
  ...
  TEMP_DIR: string;
  LOGS_DIR: string;
  TERMINAL_DIR: string; // Legacy alias for terminal storage
  TERMINALS_DIR: string; // Primary terminal storage location
  WORKSPACES_DIR: string; // Legacy alias for active workspaces
...
class PathConfiguration {
  ...
  private initializePaths(): PathConfig {
    ...
    const dataRoot = path.join(varRoot, 'data');
    const storageRoot = path.join(dataRoot, 'storage');
    const maibarnRoot = process.env.MAIBARN_ROOT || path.join(varRoot, 'maibarn');
    // CRITICAL FIX: Resolve /tmp symlink to actual path (/private/tmp on macOS)
    // This ensures consistent session visibility across all tmux operations
    const defaultTmuxTmpDir = fs.realpathSync('/tmp');
    ...
    const config: PathConfig = {
      ...
      TMUX_TMP_DIR: defaultTmuxTmpDir,
      ...
      HARVEST_DIR: path.join(maibarnRoot, 'harvests'),
      HARVESTS_DIR: path.join(maibarnRoot, 'harvests'),
      
      // Barn storage
      BARN_STORAGE: path.join(storageRoot, 'barn'),
      BARN_ITEMS: path.join(storageRoot, 'barn', 'items'),
      BARN_TEMPLATES: path.join(storageRoot, 'barn', 'templates'),
      BARN_CATALOG: path.join(storageRoot, 'barn', 'catalog.json'),
      BARN_DIR: path.join(storageRoot, 'barn'),
      ...
      TERMINAL_DIR: path.join(maibarnRoot, 'terminals'),
      TERMINALS_DIR: path.join(maibarnRoot, 'terminals'),
      WORKSPACES_DIR: path.join(maibarnRoot, 'workspaces', 'active'),
    ...
    // CRITICAL FIX: Force process-wide TMUX_TMPDIR to /tmp for all tmux operations
    // This ensures session creation and health monitoring use the same location
    process.env.TMUX_TMPDIR = config.TMUX_TMP_DIR;
  ...
  async ensureDirectoriesExist(): Promise<void> {
    ...
    for (const dir of dirsToCreate) {
      ...
      if (dir === this.config.TMUX_TMP_DIR && dir === '/tmp') {
        // Use system tmpdir as-is
        continue;
      }
      try {
        await fs.mkdir(dir, { recursive: true });
        if (dir === this.config.TMUX_TMP_DIR) {
          await fs.chmod(dir, 0o1777);
        }
      } catch (error: any) {
