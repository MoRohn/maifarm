...
export class WebSocketServer {
  ...
  private setupEventHandlers() {
    ...
    this.io.on('connection', (socket: AuthenticatedSocket) => {
      ...
      socket.on('terminal:join', (data: { farmId: string; sessionId?: string }) => {
        console.log(`[WebSocket] Terminal join (simple) request from ${socket.id}:`, data);

        // Convert to the expected format for the handler
        const normalizedData = {
          sessionId: data.sessionId || `farm-${data.farmId.substring(0, 8)}`,
          farmId: data.farmId
        };
        ...
        const { sessionId, farmId } = normalizedData;
        const terminalRooms = [];
        ...
        console.log(`[WebSocket] Socket ${socket.id} joined terminal rooms via simple join:`, terminalRooms);
        ...
        // Call the handler to start streaming
        this.terminalHandlers.handleTerminalJoinSession(socket, normalizedData);
      });

      // Support terminal:join:simple for test compatibility
      socket.on('terminal:join:simple', (data: { farmId: string; sessionId?: string }) => {
        console.log(`[WebSocket] Terminal join:simple request from ${socket.id}:`, data);
        ...
        // Delegate directly to the terminal handler
        this.terminalHandlers.handleTerminalJoinSession(socket, normalizedData);
      });
      ...
      // Handle polling request for terminal output
      socket.on('terminal:request_output', async (data: { sessionId: string; farmId?: string; agentCount?: number }) => {
        try {
          const { sessionId, farmId, agentCount = 1 } = data;
          console.log(`[Socket] Polling request for output: session=${sessionId}, agentCount=${agentCount}`);

          // Trigger file watcher to check for new output
          if (farmId) {
            const terminalFileWatcherService = await import('../services/terminalFileWatcherService').then(m => m.terminalFileWatcherService);
            await terminalFileWatcherService.checkForUpdates(farmId);
          }

          // Also attempt to capture latest pane output for each agent
            ...
            captureProcess.on('exit', (code) => {
              if (code === 0 && output && output.trim()) {
                ...
                if (lines.length > 0) {
                  ...
                  console.log(`[Socket] Sent ${lines.length} lines from poll for agent ${agentId}`);
                }
              }
            });

            captureProcess.on('error', (error) => {
              console.debug(`[Socket] Failed to capture output for agent ${agentId}:`, error.message);
            });
          }
        } catch (error) {
          console.error('Terminal request output error:', error);
        }
      });
