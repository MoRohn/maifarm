  ...
  middleware() {
    return async (req: AuthRequest, res: Response, next: NextFunction) => {
      ...
      if (bucket.tokens < cost) {
        ...
        logger.warn(LogCategory.GENERAL, `Rate limit exceeded for ${key}`);

        // Extract provider information from request if available
        const provider = (req as any).body?.provider || (req as any).query?.provider || 'claude';

        // Provider-specific URLs for adding credits
        const providerUrls = {
        ...
        const providerUrl = providerUrls[provider as keyof typeof providerUrls] || providerUrls.default;
        ...
        return res.status(429).json({
          ...
          error: {
            ...
            message: `${providerName} API rate limit exceeded. Please add credits to your ${providerName} account to continue.`,
            userMessage: `You've reached the rate limit for ${providerName}. To continue using MaiFarm, please add credits to your ${providerName} account.`,
...
export const farmCreationRateLimit = new RateLimiter({
  ...
  maxRequests: 100,         // 100 farms per hour (increased for development)
