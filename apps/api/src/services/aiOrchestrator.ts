...
class AIOrchestrator extends EventEmitter {
  ...
  private providerStats: Map<'claude' | 'openai', { requests: number; errors: number }> = new Map();
  ...
  private initialize() {
    ...
    this.providerStats.set('openai', { requests: 0, errors: 0 });
  ...
  private startHealthMonitoring() {
    ...
    this.healthCheckInterval = setInterval(async () => {
      const providers: AIProvider[] = [AIProvider.CLAUDE, AIProvider.OPENAI];
  ...
  async submitTask(request: AITaskRequest): Promise<AITaskResponse> {
    ...
    try {
      ...
      const provider = (request.provider || aiProviderManager.getDefaultProvider()) as AIProvider;
      
      // Check if provider is available
      const providerEnum = provider === AIProvider.OPENAI ? AIProvider.OPENAI : AIProvider.CLAUDE;
      ...
      const response = await aiProxy.sendRequest(aiRequest, providerEnum);
   ...
   */
  async switchFarmProvider(farmId: string, provider: 'claude' | 'openai'): Promise<void> {
    // Check if provider is available
    const providerEnum = provider === 'openai' ? AIProvider.OPENAI : AIProvider.CLAUDE;
    if (!aiProviderManager.isProviderEnabled(providerEnum)) {
      throw new Error(`Provider ${provider} is not available or configured`);
    }
   ...
   * Get provider status
   */
  async getProviderStatus(provider: 'claude' | 'openai'): Promise<ProviderStatus> {
    const providerEnum = provider === 'openai' ? AIProvider.OPENAI : AIProvider.CLAUDE;
