  ...
  private async storeApiKeyInDatabase(service: string, apiKey: string, name: string): Promise<void> {
    try {
      ...
      if (existing.rows.length === 0) {
        ...
        // Use provider value directly for service column (no mapping needed)
        await db.query(`
          INSERT INTO api_keys (name, service, provider, encrypted_key, key_encrypted, key_hash, is_active)
          VALUES ($1, $2, $3, $4, $5, $6, $7)
        `, [
          name || `${provider} API Key`,  // Provide a default name if not specified
          provider,  // service column (same as provider)
