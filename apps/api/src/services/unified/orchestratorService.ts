...
export class OrchestratorService extends EventEmitter {
   ...
   * Stop a farm
   */
  async stopFarm(farmId: string, graceful = true, userId: string = 'system'): Promise<void> {
    if (graceful) {
      await farmService.gracefulShutdownFarm(farmId, userId, 'user_request');
      return;
    }

    await farmService.stopFarm(farmId, userId);
  }
  ...
  /**
   * Get orchestrator status (used by ShutdownCoordinator)
   */
  async getStatus(farmId: string): Promise<{
    isRunning: boolean;
    agents: any[];
    processId?: string;
  }> {
    try {
      ...
      const isRunning = ['active', 'running', 'launching'].includes(farm.status);
  ...
  /**
   * Send command to agent (used by ShutdownCoordinator)
   */
  async sendCommandToAgent(sessionName: string, agentIndex: number, command: string): Promise<void> {
    try {
