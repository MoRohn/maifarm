...
import { MonitoringService as UnifiedMonitoringService } from './monitoringService';
import { OrchestratorService as UnifiedOrchestratorService } from './orchestratorService';
...
class ServiceRegistry {
  ...
  public async initialize(): Promise<void> {
    if (this.initialized) {
      logger.info(LogCategory.SYSTEM, 'Service Registry already initialized');
      return;
    }
    ...
    try {
      ...
      const orchestrationInits: Array<Promise<unknown>> = [];
      if (typeof this.farm.initialize === 'function') {
        orchestrationInits.push(this.farm.initialize());
      }
      if (typeof (this.harvest as any).initialize === 'function') {
        orchestrationInits.push((this.harvest as any).initialize());
      }
      if (typeof this.orchestrator.initialize === 'function') {
        orchestrationInits.push(this.orchestrator.initialize());
      }

      await Promise.all(orchestrationInits);
