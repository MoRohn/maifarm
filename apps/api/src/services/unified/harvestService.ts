...
export class UnifiedHarvestService extends EventEmitter {
   ...
   */
  public getHarvestById(harvestId: string): HarvestData | undefined {
  ...
  public async collectFiles(farmId: string, harvestId: string): Promise<void> {
    try {
      ...
      // Get workspace path
      const workspacePath = getPath('WORKSPACE_DIR', farmId);
      if (!workspacePath || !existsSync(workspacePath)) {
        logger.warn(LogCategory.HARVEST, `Workspace not found for farm ${farmId}`);
        return;
      }

      // Collect workspace files and terminal outputs
      const [workspaceFiles, terminalOutputs] = await Promise.all([
        this.collectWorkspaceFiles(workspacePath, harvest),
        this.collectTerminalOutputs(farm, harvest)
      ]);
  ...
  /**
   * Get harvest by ID (used by ShutdownCoordinator)
   */
  public getHarvestById(harvestId: string): HarvestData | undefined {
    return this.harvests.get(harvestId);
  }
